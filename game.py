...
class _Vertex:
    ...
    Note: Right Ascension and Declination are celestial coordinates, similar to latitude / longitude, specifically for stars and objects in the sky. 
    ...
    def __init__(
        ...
        Args:
            item:       Unique identifier (e.g. HIP number or name string).
    ...
    def angular_distance(self, other: _Vertex) -> float:
        ...
        Uses the spherical law of cosines, which is accurate for all separations.
    ...
    def similarity_score(self, other: _Vertex) -> float:
        """Return a Jaccard-style structural similarity score ∈ [0, 1].
 
        The score is based on the set of *neighbour items*, mirroring the
        original book-review implementation so that similarity.py can call
        this on constellation subgraphs.
...
class WeightedGraph:
    ...
    def __init__(self) -> None:
        """Initialise an empty graph (no vertices or edges)."""
        self._vertices = {}
    ...
    def add_vertex(
        ...
        Args:
            item:       Unique identifier for the star.
            ra:         Right Ascension in decimal degrees.
            dec:        Declination in decimal degrees.
            magnitude:  Apparent visual magnitude.
            name:       Optional human-readable name.
    ...
    def add_edge(
        ...
        If *weight* is not supplied, the weight is computed automatically as
        the angular distance between the two stars.
 
        Raise ValueError if either item is not a vertex in this graph.
        Raise ValueError if item1 == item2 (self-loops not allowed).
 
        Args:
            item1:  Identifier of the first star.
            item2:  Identifier of the second star.
            weight: Edge weight (angular degrees). Auto-computed when None.
        """
        if item1 == item2:
            raise ValueError("Self-loops are not allowed.")
        if item1 not in self._vertices or item2 not in self._vertices:
            raise ValueError(
                f"One or both vertices not found: {item1!r}, {item2!r}"
            )
    ...
    def adjacent(self, item1: Any, item2: Any) -> bool:
        """Return whether *item1* and *item2* share an edge.
